...
> 📎 **本题与 Day3 Q35 主题相同，已合并**。完整权威答案（含 thinking budget 三档分级、流式输出思考过程、超时降级机制、思考预算拐点找法、面试官追问）见 **[`Day3_大模型基础知识与推理优化.md` Q35](./Day3_大模型基础知识与推理优化.md)**。
>
> **核心结论**：推理模型和普通模型的部署差异在三点——① **internal CoT**（思考链隐藏但占 token）；② **thinking budget** 是成本/质量旋钮，需按任务难度分级；③ **延迟分布完全不同**（思考期 10-60s 无输出，必须流式吐思考缓解体感）。生产要点：成本路由（80% 普通 / 20% 推理）+ thinking budget 分级 + 超时降级 fallback，否则成本爆炸。
>
> **典型量化**：DeepSeek-R1 思考 token 占总 token 75%，单次成本是 GPT-4o 的 8 倍；难题可能想 3 分钟，网关超时需设 120-300s。
...
> 📎 **本题与 Q25 主题相同，已合并**。完整权威答案（含 KV 传输方案 Mooncake/DeepSeek 3FS/Splitwise 对比、GPU 配比、Continuous Batching、面试官追问）见 **[Q25](#q25-pd-分离工程化--2026-高频prefill-decode-分离什么时候该上kv-传输层怎么设计)**。
>
> **核心结论**：LLM 推理分 Prefill（计算密集，吃算力）和 Decode（访存密集，吃带宽）两阶段，资源特征完全不同，传统合并部署资源利用率仅 30-50%。PD 分离把两阶段拆到不同实例（Prefill 用高算力 GPU、Decode 用高带宽 GPU），各自最优配置，资源利用率提到 70-90%、成本降 40-60%。核心难点是 **KV Cache 跨节点传输**（128K 上下文的 KV 达几十 GB），用 RDMA 共享池（Mooncake 思路）解决。
>
> **判断**：高并发 + 长上下文场景是标配优化（P99 可从 15s 降到 3s）；但 QPS < 100 时单体 vLLM 更省心，别过度工程。
...
**关于 mask 的说明（面试常被追问）**：
- 上述伪代码省略了 causal mask。实际中：
  - **Decode 阶段**（T=1，传 kv_cache）：新 token 只看历史，无需 mask
  - **Prefill 阶段**（kv_cache=None，T=完整序列）：必须加 causal mask（`attn = attn.masked_fill(mask==0, -inf)`），否则未来 token 会 attend 到未来，自回归失效
- 完整实现应增加 `mask` 参数：`def forward(self, x, kv_cache=None, mask=None)`，Prefill 传 causal mask，Decode 传 None
...
- "GQA 怎么改？" → K/V 的 head 数少于 Q（如 Q 32 头，K/V 8 头），用 repeat_interleave 把 K/V 头复制到与 Q 对齐后再 attention（GQA ratio = 32/8 = 4）
...
> 📎 **本题与 Q26 主题相同，已合并**。完整权威答案（含 rank 选择、adapter 版本管理、多 adapter 并发计算、Punica/SGLang/vLLM 支持情况、热加载 vs 预加载策略）见 **[Q26](#q26-multi-lora-serving--2026-高频多租户场景怎么低成本做模型定制multi-lora-热加载怎么实现)**。
>
> **核心结论**：多租户 LLM 服务（每个租户/用户一个 LoRA）的核心思路是 **base 模型共享一份 + adapter 热加载**，按请求切换 adapter，而不是每个 LoRA 部署一份完整模型。热门 adapter 常驻显存（LRU 淘汰冷门），把用同一 adapter 的请求合批最大化吞吐。"服务几百个 LoRA 的成本约等于一个 base 模型"（MLSys 2024）。
>
> **典型效果**：ToB 客服 100 个企业客户各一个 LoRA，从 100 个模型实例（GPU 成本爆炸）改成一个 Qwen-7B base + 100 个 adapter（每个几十 MB）LRU 缓存，**GPU 成本降 95%**，热 adapter 切换 <10ms。
...
### Q30. 【成本合理性 · 2026】为什么需要这么高的成本？用 API 不行吗？

> 📎 **本题是"自建 vs API"决策框架（见 [Q14](#q14权衡决策自建推理集群-vs-用-apigptclaude什么时候自建才划算)）在 Coding Agent 场景的具体应用**。Q14 讲通用决策（盈亏平衡点约 1 亿 token/月）；本题讲 Coding Agent 特有的成本结构论证。若面试官问的是通用"自建 vs API 怎么选"，先答 Q14；若追问具体场景的成本合理性，用本题。

**① 原理**：
Coding Agent 的成本结构跟普通 chat 完全不同——单次任务平均 50-200 轮 tool call，每轮带完整上下文（工具列表 + 历史 + 记忆 + 检索结果），单次任务的 token 消耗是普通 chat 的 **50-100 倍**。面试官问"为什么这么贵"真正考的是：**你算过账吗？贵的部分花在哪？有没有 cheaper alternative？**

**② 成本拆解（月度）**：

| 成本项 | 占比 | 月费用（估算） | 说明 |
|---|---|---|---|
| 推理算力（GPU） | 60% | ~$2 万（8×H100 单机） | FP8 部署 Qwen2.5-72B |
| 数据标注 + 合成 | 15% | ~$5 千 | 持续迭代训练数据 |
| 评估基础设施 | 10% | ~$3 千 | golden set 跑评估、LLM-judge |
| 监控 + 可观测 | 10% | ~$3 千 | Langfuse、日志、告警 |
| 存储（向量库 + 对象存储） | 5% | ~$1.5 千 | 记忆库、checkpoint |

**③ 反向辩护（核心）——对比用 GPT-4o API**：
- GPT-4o API 单次任务成本：$0.5-2（50-200 轮 × 每轮平均 $0.01）
- 假设日活 1 万用户 × 日均 5 次任务 = 月 150 万次调用
  - 走 GPT-4o API：**$75 万-300 万/月**（单次 $0.5-2 × 150 万）
  - 走自研 Qwen2.5-72B：**固定 ~$5 万/月**（含算力+运维）
- **盈亏平衡点**：月调用 > 50 万次，自研就比 API 便宜
- 当前月调用 150 万次，自研比 API **省 93%+**（$75-300 万 vs $5 万）

**④ 成本不是越低越好——体验指标有成本下限**：
- **TTFT < 1s**：首 token 延迟超 1s 用户体感卡顿，这要求 GPU 不能欠配
- **长程任务可靠性 95%+**：50 步链路每步 95% → 整体 7.6%，必须用强模型 + 重试 + 校验，成本降不下来
- **成本 vs 延迟 vs 质量"不可能三角"**：我们选了"质量优先"——因为 Coding Agent 错一次用户就流失，省的钱不够弥补流失

**⑤ Tradeoff 与混合策略**：
- **纯自研的代价**：模型迭代慢（基模升级要重新微调），初期质量不如 GPT-4o
- **纯 API 的代价**：成本随规模线性增长，数据不出域难（合规问题）
- **混合策略（推荐）**：长尾/低价值任务走 API（Claude Haiku / GPT-4o-mini），高频头部任务走自研——既控成本又保体验

**示例**：
> "我们算过账：Coding Agent 单次任务 50-200 轮 tool call，走 GPT-4o API 单次 $0.5-2，月 150 万次调用就是 $75-300 万。自研 Qwen2.5-72B 部署固定 ~$5 万/月（8×H100 单机 FP8），月调用 > 50 万次就回本，现在我们月 150 万次，比 API 省 93%+（$75-300 万 vs $5 万）。但成本不是唯一考量——TTFT < 1s、长程任务可靠性 95%+ 这些体验指标都有成本下限，砍成本砍到 TTFT 2s 用户就跑了。我们也做了混合策略：路由层用 Qwen-7B 小模型分类，简单任务（如语法补全）走 Haiku API，复杂任务（重构/Debug）走自研 72B，整体再降 30%。判断：Coding Agent 这个场景，自研是规模到了之后的必然选择，小规模用 API 验证 PMF，规模化必上自研。"

🎯 **面试官追问**：
- "调用没到 50 万次怎么办？" → 走 API 验证 PMF；到了 50 万再上自研——这是标准路径，不是一上来就自研
- "能不能用更小模型降本？" → 路由层用 8B，主推理用 70B；简单任务降级到 Haiku——但主推理不能用 8B，代码质量差距大
- "MoE 不是更便宜吗？" → MoE 算力便宜但显存贵（总参数大），单机放不下 70B 级 MoE；Dense 70B 单机 8×H100 即可，ROI 更优
- "推理成本一直在降，以后会不会不划算自研？" → 算力降但数据/评估/监控的工程成本不降；且自研有数据主权和合规价值，不只是钱

⚠️ **踩坑提示**：
- 别只讲成本不辩护——这题面试官真正考的是"你算过账吗"，必须给出具体数字和盈亏平衡点
- 漏讲"质量优先"立场——会让面试官觉得你不懂体验经济，Coding Agent 不是越便宜越好
- 不提"混合策略"——显得思考维度单一，生产几乎没有纯自研或纯 API 的
